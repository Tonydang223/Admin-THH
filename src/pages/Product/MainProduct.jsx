...
export default function MainProduct() {
  ...
  const start = () => {
    ...
    setTimeout(async () => {
      if(recycles) {
        await deleteProduct({ ids: selectedRowKeys }).unwrap()
      } else {
        await deleteProductRestore({ ids: selectedRowKeys }).unwrap();
      }
      setSelectedRowKeys([]);
      setLoading({
        ...loading,
  ...
  const startRestoreSelected = () => {
    ...
    setTimeout(async () => {
      await deleteProductRestoreBack({ ids: selectedRowKeys }).unwrap();
      setSelectedRowKeys([]);
      setLoading({
        ...loading,
        reSel: false,
        ...
        <Space size="middle">
            ...
            onClick={() => {
              showGlobal({
                ...
                onOk: () => {
                  ...
                  setTimeout(async () => {
                    if(recycles) {
                      await deleteProduct({ids: dt}).unwrap();
                    } else {
                      await deleteProductRestore({ ids: dt }).unwrap();
                    }
                    setSelectedRowKeys([]);
                  }, 1000);
                },
          ...
          <Button
            ...
            onClick={async () => {
              if (recycles) {
                const dt = [product._id];
                await deleteProductRestoreBack({ ids: dt }).unwrap();
                setSelectedRowKeys([]);
              } else {
